...
module.exports = function(invApiGroup) {
	...
	invApiGroup.addroute('/types/:typeIDs', function(req, db, apiMasks, responseFn) {
		...
		db.inv.getTypeByID(queryParams, responseFn);
	});
	...
	invApiGroup.addroute('/test/:typeID', function(req, db, apiMasks, responseFn) {
		var queryParams = {
			typeID: req.params.typeID
		};

		db.inv.getTypeAttrEff(queryParams, responseFn);
	});
	...
	invApiGroup.addroute('/types/full/:typeID', function(req, db, apiMasks, responseFn) {
		...
		var typeQueryParams = {
			typeID: req.params.typeID,
			fields: apiMasks.typeMask.getFields(req.query.fields) //All fields
		};

		var attrQueryParams = {
			typeID: req.params.typeID,
			fields: apiMasks.attributeTypesMask.getFields(req.query.attributefields)
		};

		var effQueryParams = {
			typeID: req.params.typeID,
			fields: apiMasks.effectMask.getFields(req.query.effectfields)
		};

		console.log(apiMasks.attributeTypesMask.getFields(req.query.attributefields) + " " + apiMasks.effectMask.getFields(req.query.effectfields));

		db.inv.getTypeByID(typeQueryParams, function(err, typeResult) {

			if (err) { return responseFn(err); }

			if (typeResult.length > 0) {
				db.dgm.getTypeAttributesByTypeID(attrQueryParams, function(err, attributesResult) {
					
					if (err) { return responseFn(err); }

					db.dgm.getTypeEffectsByTypeID(effQueryParams, function(err, effectsResult) {

						if (err) { return responseFn(err); }
						
						typeResult[0].attributes = attributesResult;
						typeResult[0].effects = effectsResult;

						responseFn(undefined, typeResult);
