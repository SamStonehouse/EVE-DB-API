...
module.exports = function(invApiGroup) {
	...
	invApiGroup.addroute('/types/attributes/:typeID', function(req, db, apiMasks, responseFn) {
		
		var typeQueryParams = {
			typeID: req.params.typeID,
			fields: apiMasks.typeMask.getFields() //All fields
		};
	...
	invApiGroup.addroute('/types/effects/:typeID', function(req, db, apiMasks, responseFn) {
		...
		db.inv.getTypeByID(typeQueryParams, function(err, typeResult) {
			...
			if (typeResult.length > 0) {
				db.dgm.getTypeEffectsByTypeID(attrQueryParams, function(err, effectsResult) {
					if (err) {
						return responseFn(err);
					}
