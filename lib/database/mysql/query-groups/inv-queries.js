...
module.exports = function(setupQuery) {
	...
	queries.getAllGroups = setupQuery("SELECT ?? FROM invGroups WHERE published = 1", ['fields']);

	queries.getGroupByID = setupQuery("SELECT ?? FROM invGroups WHERE published = 1 AND groupID = ?", ['fields', 'groupID']);

	queries.getGroupsInCategory = setupQuery("SELECT ?? FROM invGroups WHERE published = 1 AND categoryID = ?", ['fields', 'categoryID']);

	queries.getAllCategories = setupQuery("SELECT ?? FROM invCategories WHERE published = 1", ['fields']);

	queries.getCategoryByID = setupQuery("SELECT ?? FROM invCategories WHERE published = 1 AND categoryID = ?", ['fields', 'categoryID']);
	...
	queries.getTypeByID = setupQuery("SELECT ?? FROM invTypes WHERE published = 1 AND typeID = ?", ['fields', 'typeID']);

	queries.getTypesInGroup = setupQuery("SELECT ?? FROM invTypes WHERE published = 1 AND groupID = ?", ['fields', 'groupID']);
