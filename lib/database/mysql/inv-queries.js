...
var getAllFlags = function(properties, fn) {
	var query = "SELECT ?? FROM invFlags";
	connection.query(query, [properties.fields], fn);
};

var getFlagByID = function(properties, fn) {
	var query = "SELECT ?? FROM invFlags WHERE flagID = ?";
	connection.query(query, [properties.fields, properties.flagID], fn);
};

var getMetaGroups = function(properties, fn) {
	var query = "SELECT ?? FROM invMetaGroups";
	connection.query(query, [properties.fields], fn);
};
